    ...
    def link_to_batch(self, request, pk=None):
        ...
        try:
            ...
            batch = get_object_or_404(Batch, id=batch_id)
              # Link the manifest to the batch using receipt_batch field
...
class ManifestGroupViewSet(viewsets.ReadOnlyModelViewSet):
    """
    API endpoint for viewing manifest groups.
    
    Provides read-only access to groups of similar items from a manifest.
    ...
    def set_product_family(self, request, pk=None):
        ...
        POST /api/manifests/groups/{id}/set_product_family/
        """
        ...
        try:
            # Get the product family by ID
            product_family = ProductFamily.objects.get(id=family_id)
            
            # Update the direct foreign key
