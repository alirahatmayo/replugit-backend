...
# def update_warranty_status_for_order(order, new_state):
#     """
#     Update the warranty statuses for all items in an order based on the new state.
#     Prevents recursive signals from triggering infinite loops.
#     """
#     try:
#         # ✅ Dynamically fetch OrderItem model
#         OrderItem = apps.get_model('orders', 'OrderItem')

#         # ✅ Temporarily disable signals to prevent recursion
#         from orders.signals import handle_product_unit_assignment
#         post_save.disconnect(handle_product_unit_assignment, sender=OrderItem)

#         with transaction.atomic():
#             for item in order.items.prefetch_related('assigned_units').all():
#                 if not item.assigned_units.exists():
#                     continue

#                 for unit in item.assigned_units.all():
#                     warranty = Warranty.objects.filter(product_unit=unit).first()
#                     if not warranty:
#                         continue
...
#                         elif new_state in ['shipped', 'delivered']:
#                             warranty.transition_status('active')

#                         warranty.save()
#                         logger.info(f"Updated warranty for unit {unit.serial_number} to {warranty.status}")

#                     except Exception as e:
#                         logger.error(f"Failed to update warranty for ProductUnit {unit}: {e}")

#     finally:
#         # ✅ Reconnect signals after processing
#         post_save.connect(handle_product_unit_assignment, sender=OrderItem)
